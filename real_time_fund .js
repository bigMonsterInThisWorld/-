...
jQuery(function () {
    ...
    jQuery.ajax({
        ...
        success: function () {
            try {
                if (!(typeof aff_data == "undefined" || aff_data == null || typeof aff_data.data == "undefined")) {
                    if (aff_data.data.length > 0) {
                        jQuery("#flash-cont-2,#flash-cont-3").html("");
                        zjlx.data = aff_data.data[0].reverse();
                        if (zjlx.data.length > 0) {
                            var date = new Date(zjlx.data[0].split(',')[0]);
                            var hhmm = getDate(Now).Format("hhmm") * 1;
                            var rq = date.Format("yyyy-MM-dd");

                            if (hhmm < 1500 && rq == getDate(Now).Format("yyyy-MM-dd")) {
                        ...
                        var $Html = '<span style="display: block;text-align: center;padding-top: 100px;">没有相关数据</span>';
                        jQuery("#flash-cont-2,#flash-cont-3").html($Html);
                    }
                }
                else {
                    var $Html = '<span style="display: block;text-align: center;padding-top: 100px;">没有相关数据</span>';
    ...
    tiny.loadJs(b, "utf-8",
        function () {
            if (!(typeof zjlx_hq == "undefined" || zjlx_hq == null)) {
                c = zjlx_hq.split(",");
                if (c.length < 16) {
                    c = [_stockMarke, _stockCode, _stockName, "-", "-", "-", "-", "-", "-", "-", "-", "-", "-", "-", "-", "-"]
                }
                d = tableCache.replace(/{code}/ig, c[1])
                    ...
                    .replace(/{zf}/ig, c[8])
                    .replace(/{amount}/ig, c[6])
                    .replace(/{volume}/ig, c[7] == "-" ? "-" : (c[7] / 10000).toFixed(2))
    ...
    tiny.loadJs(b, "utf-8", function () {
        if (!(typeof tab_data == "undefined" || tab_data == null)) {
            ...
            if (dataArr.length == 1) {
                ...
                //算和值
                var data = [];
                data[0] = (_dh[5] == "" || _dh[5] == "-") ? "0.0000" : ((parseFloat(_dh[5]))).toFixed(4);//今日主力净流入
...
function DataTable() {
    ...
    var tablelist = new LoadTable({
        ...
        maketr: function (row, data, i, c) {
            ...
            //日期
            var Tdate1 = data[0] != "" ? getDate(data[0]).Format("yyyy-MM-dd") : "-";
            var tdatetitle1 = data[0] != "" ? getDate(data[0]).Format("yyyy-MM-dd") : "-";
            row.cells[0].innerHTML = "<span title='" + tdatetitle1 + "' >" + Tdate1 + "</span>";
            //收盘价4
            var spj = isempty(data[11]) ? '-' : (data[11] / 1).toFixed(2);
            row.cells[1].innerHTML = "<span class='" + getColor(data[12].replace("%", "") / 1) + "' style='font-weight:normal'>" + spj + "</span>";
            //涨跌幅12
            var qjzdf = isempty(data[12]) ? '-' : data[12];
            ...
            //主力净流入净占比
            var zljlrzb = isempty(data[2]) ? '-' : data[2];
            ...
            //超大单净流入净占比
            var cdjlrzb = isempty(data[3]) ? '-' : data[4];
            ...
            //大单净流入净占比
            var ddjlrzb = isempty(data[6]) ? '-' : data[6];
            ...
            //中单净流入净占比
            var zdjlrzb = isempty(data[8]) ? '-' : data[8];
            ...
            //小单净流入净占比
            var xdjlrzb = isempty(data[10]) ? '-' : data[10];
...
var opChart = {
    ...
    dataTransfer: function (obj, danwei) {
        ...
        var xArr = (obj.xa != null && typeof obj.xa != "undefined") ? obj.xa.split(",") : [];
        ...
        var yArr = (obj.ya != null && typeof obj.ya != "undefined") ? obj.ya : [];
    ...
    initTotal: function (code) {
        ...
        jQuery.ajax({
            type: "get",
            url: "http://ff.eastmoney.com/EM_CapitalFlowInterface/api/js?id=" + code + "&type=ff&check=MLBMS&cb=var%20aff_data=&js={(x)}&rtntype=3&acces_token=1942f5da9b46b069953c873404aad4b5",
            dataType: "script",
            scriptCharset: "utf-8",
            success: function () {
    ...
    phTotal: function (code) {
        ...
        jQuery.ajax({
            type: "get",
            url: "http://ff.eastmoney.com/EM_CapitalFlowInterface/api/js?id=" + code + "&type=hff&rtntype=2&js=(x)&check=TMLBMS&cb=var%20ph_data=&style=top&num=42&_=1482756720594&acces_token=1942f5da9b46b069953c873404aad4b5",
            dataType: "script",
            scriptCharset: "utf-8",
            success: function () {
...
function isempty(srcdata) {
    if (srcdata == "" || srcdata == "-") return true;
    else return false;
}